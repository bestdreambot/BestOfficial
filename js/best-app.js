...
(function(){
  ...
  // этого релиза.
  // 07.08.2026, Андрей: «выпусти Релиз 4.1 как закрепляющий момент» — не полный Релиз 5 (два
  // открытых вопроса ниже не решены), а промежуточная точка фиксации всего, что уже готово.
  // red здесь — «Планы» (исходный смысл поля, не блокер релиза): то, что не остановило выпуск
  // 4.1, но остаётся открытым для настоящего Релиза 5.
  var RELEASE_INFO = {
    title: 'Процесс',
    readyStatus: 'ready',
    readiness: '🟢 Релиз 4.1 выпущен — закрепляющий момент. Полный Релиз 5 ждёт двух открытых вопросов ниже.',
    yellow: [],
    red: [
      'Новое имя для Valmont — ещё не найдено',
      'Лучший световой элемент для тора BestOfficial — отдельный вопрос, тоже не найден'
    ]
  };
  ...
  var VERSION_INFO = {
    title: 'Процесс',
    session: '07.08.2026 — после Релиза 4: 2 реальных бага в самом продукте исправлено (TikTok Алины ' +
      'открывал чужой аккаунт, TikTok Coffee вёл на мёртвую ссылку), архив приведён в порядок в ' +
      '6+ файлах, конфликты разбираем по одному с прямым подтверждением от Андрея.',
    green: [
      '🔢 Итог: 2 бага в продукте исправлено (TikTok Алины, TikTok Coffee) — оба подтверждены Андреем напрямую, не архивной догадкой',
      'Карточка «Версия»/«Релиз» теперь честно показывает, готовы ли мы к следующему релизу — прямой цветной вердикт сверху, не спрятан внутри списка (этого не было в Релизе 4)',
      'Все спорные адреса и данные звёзд выверены напрямую с Андреем: Алина, Дима, никнеймы Гали/Маши/Саши — надёжнее, чем архивные догадки Релиза 4',
      'Добавлен новый проект Exclusive Man — войдёт в цепочку Релиза 5',
      'Все 15 карточек участников в BEST/участники проекта/ заполнены живыми данными (было в основном пусто), Виталию впервые заведена своя карточка',
      'Medium-тор (живая уменьшенная копия главного тора) работает на карточках проектов и звёзд',
      'Записана честная хронология тора BEST — три отдельных уровня (0 / купленный и доработанный «Бест тор» / наш Ктор), чтобы будущие решения по дизайну не путали разные находки между собой',
      'Эталон уже опубликован — GitHub (bestdreambot/BestOfficial) и bestofficial.vercel.app, это и есть Релиз 4',
      'В CLAUDE.md зафиксировано постоянное правило: адрес/якорные данные меняются только по прямому слову Андрея, архивная гигиена — постоянный процесс, не разовая уборка',
      'Уточнено: ни ушедшие участники (Марина Анирам и другие), ни отсутствие руководителя у части проектов, ни аудит от Kilo/Copilot — не задачи этого релиза, всё переехало в глобальные планы Андрея («когда-нибудь вернусь к аудитам»)',
      'Medium-тор Андрей проверил лично — и на проектах, и на звёздах, всё в порядке, вопрос закрыт',
      'Дубль архива (BEST/ vs projects/BEST/) — уточнено у Андрея: ничего важного не теряется, живая версия (projects/BEST/) уже полнее старой, менять не к спеху',
      'Свежие фото от звёзд — решили вообще не отслеживать как задачу, это личное дело каждой звезды: захочет — пришлёт'
    ],
    yellow: [
      'По каждому проекту/звезде — какой у них уровень тора (0 / «Бест тор») и в каком цвете — разбираем по одному, не всё разом'
    ],
    red: [
      'Новое имя для Valmont вместе с Олесей — ещё не найдено',
      'Лучший световой элемент для тора BestOfficial — отдельный вопрос, не связанный с Valmont, тоже пока не найден'
    ]
  };
  ...
  // список фактов сам по себе не презентация. Ниже — та же правда, но собранная в короткий рассказ:
  // кто мы → что нового → что уже крепко → что делать. Без технических слов (движок/файлы/код) —
  // только то, что реально видно и ощущается, тем же принципом, что и везде на публичной стороне.
  var PRESENTATION_INFO = [
    'BestOfficial — заглавная страница всего проекта, живая витрина Best',
    'Идёт подготовка к Релизу 5 — новый шаг вперёд',
    'Разработан уникальный тор BestOfficial',
    'Появился medium-тор — маленький живой тор теперь светится в каждой карточке, у каждого проекта и у каждой звезды',
